package com.guojie.tools.pwdEditText;
...
public class PwdEditText extends AppCompatEditText {
    ...
    private void setAttrs(AttributeSet attrs) {
        ...
        if (t != null) {
            ...
            StrokeWidth = t.getDimensionPixelSize(R.styleable.PwdEditText_strokeWidth, (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,3,getResources().getDisplayMetrics()));
            ...
            textSize = t.getDimensionPixelSize(R.styleable.PwdEditText_tSize, (int)TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP,16,getResources().getDisplayMetrics()));
            ...
            textColor = t.getColor(R.styleable.PwdEditText_tColor,0xFF000000);
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        if(mText.length()==textLength){
            OnTextChangeListener.onPasswordFinish(mText);
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        int Hide= Math.max(getMeasuredHeight(), getMeasuredWidth() / textLength);
...
//            }
            ...
            RectF rect = new RectF(i * Hide + spzceX, spzceY,i * Hide + Hide - spzceX,Wide - spzceY); //四个值，分别代表4条线，距离起点位置的线
            if(mCurrentLength==i && isFocused()){
                sidePaint.setColor(getResources().getColor(R.color.purple_200));
            }else {
                sidePaint.setColor(defaultColor);
            }
...
//                        rectFS.get(j).top + ((rectFS.get(j).bottom - rectFS.get(j).top) / 2) + textRect.height() / 2, textPaint);
    ...
    interface OnTextChangeListener{
        ...
        void onPasswordFinish(String pwd);
